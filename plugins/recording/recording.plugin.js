  ...
  function load(participants$, conferenceDetails$) {
    participants$.subscribe(participants => {
      ...
      if ( participants.filter( participant => participant.uuid === uuid && participant.isStreaming ).length > 0 ) {
        state = {
          icon: 'assets/images/stopRecording.svg#stopRecording',
        ...
        state = {
          icon: 'assets/images/startRecording.svg#startRecording',
  ...
  function startRecording() {
    PEX.pluginAPI
      .openTemplateDialog({
          ...
          body: `<form>
                <input id="simpleRecordPluginAlias" class="pex-text-input" placeholder="Type the alias here" autofocus />
      ...
      .then(dialogRef => {
        ...
        document
          ...
          .addEventListener('click', event => {
            ...
            window.PEX.pluginAPI.dialOut(alias, protocol, 'guest', value => {
